TGraphErrors * CHI5(int replay, int bin, double eMin, double eMax, double & ymin, double & ymax, TGraphErrors * &g,TGraphErrors * &gA, TGraphErrors * &gB, TGraphErrors * &gSpec,TGraphErrors * &gint, TGraphErrors *& gintA, TGraphErrors *& gintB){
  double vint = 0;
  double vinte = 0;
  double vintA = 0;
  double vintAe = 0;
  ...
  for(int i = 0; i<12; i++) {
    ...
    if(replay==Chi5) {
      gtmp  = GetVNPt(N523SUB2,bin,-1,EtaMin,EtaMax,gtmp, gtmp, gtmp, vint, vinte, vintA, vintAe, vintB, vintBe, true);
      gtmp2 = GetVNPt(D2232SUB2,bin,-1,EtaMin,EtaMax,gtmp2, gtmp2, gtmp2, vint2, vinte2, vintA2, vintAe2, vintB2, vintBe2, true);
    } else {
      gtmp  = GetVNPt(N523ASUB2,bin,-1,EtaMin,EtaMax,gtmp, gtmp, gtmp, vint, vinte, vintA, vintAe, vintB, vintBe, true);
      gtmp2 = GetVNPt(D2232ASUB2,bin,-1,EtaMin,EtaMax,gtmp2, gtmp2, gtmp2, vint2, vinte2, vintA2, vintAe2, vintB2, vintBe2, true);
    }
    fin->Close();
  ...
  //
  if(replay==Chi5) { 
    g = GetVNPt(N523SUB2,bin,-1,eMin,eMax,gA, gB, gSpec, vint, vinte,vintA, vintAe, vintB, vintBe, true);
    g2 = GetVNPt(D2232SUB2,bin,-1,EtaMin,EtaMax,gA2, gB2, gtmp2, vint2, vinte2, vintA2, vintAe2, vintB2, vintBe2, true);
  } else {
    g = GetVNPt(N523ASUB2,bin, -1,eMin,eMax,gA, gB, gSpec, vint, vinte,vintA, vintAe, vintB, vintBe, true);
    g2 = GetVNPt(D2232ASUB2,bin,-1,EtaMin,EtaMax,gA2, gB2, gtmp2, vint2, vinte2, vintA2, vintAe2, vintB2, vintBe2, true);
  }
